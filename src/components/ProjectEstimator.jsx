...
const projectTypes = [
  { id: "ai-automation", title: "AI Agents & Automation", desc: "Claude / LLM pipelines, Make.com, n8n, GHL", baseDays: 10, icon: "🤖" },
  { id: "fullstack", title: "Full Stack Web App", desc: "React/Next.js frontend + Node.js/MySQL backend", baseDays: 14, icon: "⚡" },
  { id: "saas-mvp", title: "SaaS / MVP Build", desc: "Fast-to-market production ready product", baseDays: 18, icon: "🚀" },
  { id: "frontend-ui", title: "UI/UX & Frontend", desc: "Modern animated web experience with GSAP & 3D", baseDays: 7, icon: "🎨" },
];
...
const timelines = [
  { id: "fast", label: "Fast-Track", multiplier: 0.85, note: "Priority sprint delivery" },
  { id: "standard", label: "Standard Pace", multiplier: 1.0, note: "Balanced milestone reviews" },
  { id: "flexible", label: "Iterative & Flexible", multiplier: 1.25, note: "Phased continuous deployment" },
];

export default function ProjectEstimator() {
  const [selectedType, setSelectedType] = useState(projectTypes[0].id);
  const [selectedFeatures, setSelectedFeatures] = useState(["ai-pipeline", "auth", "admin"]);
  ...
  const calculation = useMemo(() => {
    ...
    const totalDays = Math.round((pType.baseDays + featureDays) * timeline.multiplier);
  ...
  return (
    <div className="reveal mt-16 rounded-3xl glass-strong neon-ring p-6 sm:p-10 border border-cyan-400/25">
      ...
      <p className="text-slate-400 text-sm max-w-2xl mb-8 leading-relaxed">
        Select your project specs below to get an instant scope estimate, and send it directly to my inbox with one click.
      </p>
          ...
          {projectTypes.map((t) => {
            ...
            return (
              ...
              >
                ...
                <div className="font-display font-semibold text-white text-sm">{t.title}</div>
                <div className="text-xs text-slate-400 mt-1 leading-snug">{t.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
          {featureList.map((f) => {
            ...
            return (
              ...
              >
                <span className={`w-4 h-4 rounded-md flex items-center justify-center text-[10px] shrink-0 border ${
                  isChecked ? "bg-cyan-400 border-cyan-300 text-black font-bold" : "border-slate-600"
                }`}>
                  {isChecked ? "✓" : ""}
                </span>
            ...
            {timelines.map((tm) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-400 mt-0.5">{tm.note}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-2xl p-5 bg-gradient-to-br from-cyan-500/10 via-black/40 to-violet-500/15 border border-cyan-400/30 flex flex-col justify-between">
          <div>
            ...
            <div className="text-xs text-slate-400 mt-1">
              {calculation.estimatedDays} ({calculation.featureCount} modules selected)
            </div>
          </div>
