...
const stats = [
  ...
  { number: 25, suffix: "+", label: "Projects Shipped", icon: "🚀" },
  { number: 8, suffix: "+", label: "Certifications", icon: "🏆" },
  ...
  return (
    <section className="py-28">
      <div className="max-w-6xl mx-auto px-6 md:px-0">
        ...
        <SectionHeading label="Metrics" title="Numbers That Speak" />

        <div ref={staggerRef} className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div ref={staggerRef} className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {stats.map((stat, index) => (
            <div key={stat.label} data-stagger>
              ...
              <div className="relative overflow-hidden rounded-2xl bento-card p-6 sm:p-8 text-center group">
                <div
                  aria-hidden
                  ...
                  className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-400/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"
                />
                <div className="text-3xl mb-4 float-y">{stat.icon}</div>
                <div className="text-3xl mb-3 float-y">{stat.icon}</div>
                <StatNumber stat={stat} index={index} />
                <div className="mt-2 text-sm font-medium text-slate-400">
                <div className="mt-2 text-xs sm:text-sm font-medium text-slate-400">
                  {stat.label}
                </div>
              </div>
            </div>
