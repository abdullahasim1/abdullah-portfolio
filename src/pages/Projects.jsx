...
function CardLinks({ project, large = false }) {
  ...
  return (
    <div className="flex gap-2.5 pt-1">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`inline-flex items-center justify-center gap-2 rounded-xl glass font-medium text-slate-200 hover:text-cyan-300 hover:border-cyan-400/40 transition-colors ${pad}`}
      >
...
function FeaturedProjectCard({ project, large = false }) {
  return (
    <article
    ...
    >
      ...
      <div className={`relative z-10 flex flex-col flex-grow ${large ? "md:w-[46%] p-8 md:p-10 justify-center" : "p-6"}`}>
        ...
        <div className={`${large ? "mt-8" : "mt-auto pt-5"}`}>
          <CardLinks project={project} large={large} />
        </div>
      </div>
    </article>
  );
          ...
          {enrichedFeatured.map((project) => (
            ...
            >
              <div data-stack-inner className="will-change-transform">
                <FeaturedProjectCard project={project} large />
              </div>
            </div>
          ))}
        </div>
